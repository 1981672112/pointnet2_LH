...
跑point_cloud_transformer_cls_zj 暂时有问题
...
def main(args):
    ...
    data_path = '/home/lh/point_cloud/all_datasets/modelnet40_normal_resampled'
    ...
    classifier = model.get_model(num_class, normal_channel=args.use_normals)  # model是模块 调用里面的get_model
    criterion = model.get_loss()  # 调用里面的get_loss
    classifier.apply(inplace_relu)

    if not args.use_cpu:
        classifier = classifier.cuda()  # classifier是一个PyTorch的Module对象，它包含了整个神经网络的结构和参数。调用classifier.cuda()方法后，该模型中所有参数和计算都将被移动到当前可用的GPU上执行。如果当前没有可用的GPU，该方法会抛出异常。
        criterion = criterion.cuda()

    # exp_dir='/home/lh/point_cloud/test1_pointnet2/Pointnet_Pointnet2_pytorch-master/log/classification/pointnet2_ssg_wo_normals'
    # exp_dir='./log/classification/pointnet2_ssg_wo_normals'
    try:
        checkpoint = torch.load(str(exp_dir) + '/checkpoints/best_model.pth')  # 当前训练肯定没有.pth 所以
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=20, gamma=0.7)  # 每20个epoch 就把学习率变为上一次的0.7
    global_epoch = 0
    global_step = 0
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for batch_id, (points, target) in tqdm(enumerate(trainDataLoader, 0), total=len(trainDataLoader), smoothing=0.9):
            ...
            points = points.data.numpy()  # # 将点云数据从PyTorch张量转换为NumPy数组
            ...
            pred, trans_feat = classifier(points)  # 这是原始的 下面两行新加的
            # pred = classifier(points)
            # trans_feat = None
