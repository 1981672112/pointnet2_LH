...
class get_model(nn.Module):
    ...
    def forward(self, xyz):
        ...
        up_feature = self.first_f(norm)  # 2 64 1024
        up_position = self.first_p(xyz)  # 2 64 1024
...
class CSA_Layer(nn.Module):
    ...
    def forward(self, feature, position):
        ...
        energy = torch.sum(energy, dim=2, keepdim=False)  # b n c 列和 位置形成注意
        energy = energy / (1e-9 + energy.sum(dim=-1, keepdim=True))  # 除以行和
        attention = self.softmax(energy).permute(0, 2, 1)  # 2 128 256

        x_r = torch.mul(attention, x_v)  # b, c, n 逐点点乘
...
class CSANetSetAbstraction(nn.Module):
    ...
    def forward(self, xyz, csa_f, csa_p=None):
        ...
        csa_position = self.csa_position(position, feature)  # 交换
...
def sample_and_group(npoint, radius, nsample, xyz, points, knn, csa_p):
    ...
    new_points = index_points(points, fps_idx)  # 采样点位置和特征
    if knn:
    ...
    grouped_xyz = index_points(xyz, idx)  # 邻居点位置和特征

    # original coordinate offset
    grouped_xyz_norm = grouped_xyz - new_xyz.view(B, S, 1, C)
    grouped_points_norm = grouped_points  # - new_points.view(Bf, S, 1, Cf)  # 邻居点特征
