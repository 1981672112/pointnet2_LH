...
class Point_SACcross(nn.Module):
    def __init__(self, channels):
        super(Point_SACcross, self).__init__()
        self.v_conv = nn.Conv1d(channels, channels, 1)  # 由Linear来
        self.trans_conv = nn.Conv1d(channels, channels, 1)
        self.after_norm = nn.BatchNorm1d(channels)
        self.act = nn.ReLU()
    ...
    def forward(self, x, p):  # x 邻居最大池化后 p 选择采样点位置信息高维特征  同维度 [2,128,2048]
        # B, C, N = x.shape
        x0 = self.v_conv(x)  # 通道注意
        x1 = x.permute(0, 2, 1)
        p0 = self.v_conv(p)
        ...
        attention = self.softmax(energy)
        attention = attention / (1e-9 + attention.sum(dim=1, keepdims=True))
        x2 = torch.bmm(p0, attention)

        energy_p = torch.bmm(p1, p0)
        attention_p = self.softmax(energy_p)
        attention_p = attention / (1e-9 + attention_p.sum(dim=1, keepdims=True))
        p2 = torch.bmm(x0, attention_p)

        x_r = self.act(self.after_norm(self.trans_conv(x - x2)))
        x_r_p = self.act(self.after_norm(self.trans_conv(p - p2)))

        x = x + (self.affine_alpha * x_r + self.affine_beta) + (self.affine_alpha * x_r_p + self.affine_beta)
        return x
